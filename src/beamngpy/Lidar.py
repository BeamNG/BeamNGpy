...
class Lidar:
    ...
    def __init__(self, name, bng, vehicle=None, requested_update_time=0.1, update_priority=0.0, pos=(0, 0, 1.7), dir=(0, -1, 0), up=(0, 0, 1), 
        vertical_resolution=64, vertical_angle=26.9, rays_per_second=2200000, frequency=20, horizontal_angle=360, max_distance=120, is_using_shared_memory=True, 
        ...
        bng.open_lidar(name, vehicle, is_using_shared_memory, self.point_cloud_shmem_handle, self.point_cloud_shmem_size, self.colour_shmem_handle,
            self.colour_shmem_size, requested_update_time, update_priority, pos, dir, up, vertical_resolution, vertical_angle, rays_per_second, frequency, 
            horizontal_angle, max_distance, is_visualised, is_annotated, is_static, is_snapping_desired, is_force_inside_triangle)
    ...
    def _convert_binary_to_array(self, binary):
        ...
        ctr = 0
        for i in range(0, int(len(binary['pointCloud'])), 4):                      # Convert the binary string to a float array.
            floats[ctr] = struct.unpack('f', binary['pointCloud'][i:i + 4])[0]
            ctr = ctr + 1
        points = []
        for i in range(0, int(len(floats) / 3), 3):                                 # Convert the floats to points by collecting each triplet.
