...
class RoadNetworkExporter:
    ...
    def _compute_end_point_tangent(self, j, seg, j_key, second_key, is_this_segment_start):
        ...
        if is_less_than_limit:
            # change p0 and p2 depending on which end of the segment this is.
        ...
        for i in range(len(path_segments)):
            ...
            for j in range(len(seg) - 1):
                ...
                roads.append(Road(key1, key2, p1, line_length, heading_angle,
                             self.coords[key1].z, linear_elevation, start_width, linear_width, ref_line_cubic))
    ...
    def export_xodr(self, name):
        ...
        with open(file_name, 'w') as f:
            ...
            for i in range(len(roads)):
                ...
                f.write('\t\t\t\t<paramPoly3 aU="0.0000000000000000e+00" bU="' + str(Bu) + '" cU="' + str(Cu) + '" dU="' + str(Du) +
                        '" aV="0.0000000000000000e+00" bV="0.0000000000000000e+00" cV="' + str(Cv) + '" dV="' + str(Dv) + '"/>\n')
                f.write('\t\t\t</geometry>\n')
                f.write('\t\t</planView>\n')
