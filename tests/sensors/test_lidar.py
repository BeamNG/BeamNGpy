...
def test_lidar(beamng: BeamNGpy):
    with beamng as bng:
        ...
        sleep(2)
        data = lidar1.poll()
        print("LiDAR point cloud data (automatic polling): ", data.get("pointCloud", []))
        print("LiDAR colour data (automatic polling): ", data.get("colours", []))
        ...
        print("Ad-hoc poll request sent. Unique request Id number: ", request_id)
        sleep(3)
        # Ensure that the data has been processed before collecting.
        if lidar1.is_ad_hoc_poll_request_ready(request_id):
            data_ad_hoc = lidar1.collect_ad_hoc_poll_request(request_id)
            print("LiDAR point cloud data (ad-hoc polling): ", data_ad_hoc.get("pointCloud", []))
            print("LiDAR colour data (ad-hoc polling): ", data_ad_hoc.get("colours", []))
        else:
            print("Ad-hoc poll request not ready.")
        lidar1.remove()
        ...
        sleep(3)
        # Ensure that the data has been processed before collecting.
        if lidar2.is_ad_hoc_poll_request_ready(request_id):
        ...
        sleep(2)
        data = lidar3.poll()
        print("LiDAR point cloud data (should be zeros): ", data.get("pointCloud", []))
        print("LiDAR colour data (should be zeros): ", data.get("colours", []))
        lidar3.remove()

        # Create a LiDAR sensor with a negative requested update rate without shared memory.
        lidar4 = Lidar(
            "lidar4",
            bng,
            vehicle,
        ...
        sleep(2)
        data = lidar4.poll()
        print("LiDAR point cloud data (should be empty): ", data.get("pointCloud", []))
        print("LiDAR colour data (should be empty): ", data.get("colours", []))
        lidar4.remove()
