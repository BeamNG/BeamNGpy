...
def test_advanced_IMU(beamng: BeamNGpy):
    with beamng as bng:
        ...
        sleep(3)
        print('Is ad-hoc request complete? ',
              IMU1.is_ad_hoc_poll_request_ready(request_id))  # Ensure that the data has been processed before collecting.
        sensor_readings_ad_hoc = IMU1.collect_ad_hoc_poll_request(
            request_id)  # Collect the data now that it has been computed.
