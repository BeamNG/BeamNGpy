...
def main():
    ...
    # Create the vehicles.
    vehicle1 = Vehicle('ego_vehicle', model='pickup', licence='ego vehicle', color='Red')
    vehicle2 = Vehicle('relay_vehicle1', model='pickup', licence='vehicle2', color='Blue')
    vehicle3 = Vehicle('relay_vehicle2', model='pickup', licence='vehicle3', color='Green')

    vehicle7 = Vehicle('leader3', model='bastion', licence='vehicle7', color='Orange')
    vehicle8 = Vehicle('relayVehicle31', model='bastion', licence='vehicle8', color='Purple')
    vehicle9 = Vehicle('relayVehicle32', model='bastion', licence='vehicle9', color='Yellow')
    vehicle10 = Vehicle('relayVehicle33', model='bastion', licence='vehicle10', color='Black')

    # Create a scenario.
    scenario = Scenario('italy', 'platoontest', description='platoon') #what is platoontest

    scenario.add_vehicle(vehicle1, pos=(252.56326635601, 1205.0963008935, 169.6981158547), rot_quat=angle_to_quat((0, 0, 90)))
    scenario.add_vehicle(vehicle2, pos=(262.56326635601, 1205.7963008935, 169.69811585470), rot_quat=angle_to_quat((0, 0, 90)))
    scenario.add_vehicle(vehicle3, pos=(270.76326635601, 1206.5963008935, 169.69811585470), rot_quat=angle_to_quat((0, 0, 90)))



    scenario.add_vehicle(vehicle7, pos=(190.56326635601, 1202.0963008935, 169.6981158547), rot_quat=angle_to_quat((0, 0, 90)))
    scenario.add_vehicle(vehicle8, pos=(200.56326635601, 1202.0963008935, 169.69811585470), rot_quat=angle_to_quat((0, 0, 90)))
    scenario.add_vehicle(vehicle9, pos=(212.76326635601, 1203.0963008935, 169.69811585470), rot_quat=angle_to_quat((0, 0, 90)))
    scenario.add_vehicle(vehicle10, pos=(222.76326635601, 1203.0963008935, 169.69811585470), rot_quat=angle_to_quat((0, 0, 90)))
    ...
    speed3 = 20.0
    trafficMode = 0
    platoon_api.load(vehicle1.vid, vehicle2.vid, vehicle3.vid,  None, speed1, True) # second vehicle joins platoon
    platoon_api.load(vehicle7.vid, vehicle8.vid, vehicle9.vid, vehicle10.vid, speed3, True)


    print("Started platoon...")
    oldVelV1x = 0
    oldVelV1y = 0
    oldVelV1z = 0
    ...
    for i in range(200):
        ...
        pos1 = math.sqrt((pos1x - originV1x) **2 + (pos1y - originV1y) **2 + (pos1z - originV1z) **2) 
        ...
        pos2 = math.sqrt((pos2x - originV2x) **2 + (pos2y - originV2y) **2 + (pos2z - originV2z) **2) 
        ...
        pos3 = math.sqrt((pos3x - originV3x) **2 + (pos3y - originV3y) **2 + (pos3z - originV3z) **2) 
        ...
        pos7 = math.sqrt((pos7x - originV7x) **2 + (pos7y - originV7y) **2 + (pos7z - originV7z) **2) 
        ...
        pos8 = math.sqrt((pos8x - originV8x) **2 + (pos8y - originV8y) **2 + (pos8z - originV8z) **2) 
        ...
        pos9 = math.sqrt((pos9x - originV9x) **2 + (pos9y - originV9y) **2 + (pos9z - originV9z) **2) 
        ...
        pos10 = math.sqrt((pos10x - originV10x) **2 + (pos10y - originV10y) **2 + (pos10z - originV10z) **2) 
        ...
        idealSeparationV1V2.append(12 + 5 + (time[i]-oldTime) * abs(vel2/3.6 - vel1/3.6) + (vel2/3.6)*0.1 )
        idealSeparationV2V3.append(12 + 5 + (time[i]-oldTime) * abs(vel3/3.6 - vel2/3.6) + (vel3/3.6)*0.1 )

       
        idealSeparationV7V8.append(12 + 5 + (time[i]-oldTime) * abs(vel8/3.6 - vel7/3.6) + (vel8/3.6)*0.1 )
        idealSeparationV8V9.append(12 + 5 + (time[i]-oldTime) * abs(vel9/3.6 - vel8/3.6) + (vel9/3.6)*0.1 )
        idealSeparationV9V10.append(12 + 5 + (time[i]-oldTime) * abs(vel10/3.6 - vel9/3.6) + (vel10/3.6)*0.1 )
        ...
        diffPosV9V10.append(math.sqrt((pos9x-pos10x) ** 2 + (pos9y-pos10y) ** 2 )-5)
    ...
    ax[0,0].set(xlabel = 'Simulation Time (s)', ylabel = "Speed (Km/h)", title = 'Speed vs. Time')
    ...
    ax[0,1].set(xlabel = 'Simulation Time (s)', ylabel = "Vehicle Position (m)", title = 'Position vs. Time')
    ...
    ax[1,0].set(xlabel = 'Simulation Time (s)', ylabel = " Position difference between vehicles(m)", title = 'interVehicular Distance')
    ax[1,0].plot(time, diffPosV1V2, 'm', label="Leader and follower 1")
    ax[1,0].plot(time, diffPosV2V3, 'c', label="Follower 1 and follower2")
    ax[1,0].legend()
    
    ax[1,1].set(xlabel = 'Simulation Time (s)', ylabel = "Vehicle Acceleration (m/s^2)", title = 'Acceleration vs. Time')
