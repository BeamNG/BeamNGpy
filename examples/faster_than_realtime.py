...
def main():
    ...
    try:
        ...
        bng.control.queue_lua_command(f"settings.setValue('fpsLimit', {fpslimit})")
        bng.control.queue_lua_command("settings.setValue('fpsLimitEnabled', true)")
        bng.control.queue_lua_command(f"be:setPhysicsSpeedFactor({iterations})")
        
        print(f"Running at {factor}x speed for 30s real-time (60s simulation)...")
        time.sleep(30)
        
        # Reset to normal speed
        bng.control.queue_lua_command("be:setPhysicsSpeedFactor(0)")
