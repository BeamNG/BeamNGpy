...
def main():
    ...
    scenario.add_vehicle(
        vehicle2,
        pos=(-360.56326635601, 1169.0963008935, 168.69811585470),
        rot_quat=angle_to_quat((0, 0, 90)),
    )
    scenario.make(bng)
    ...
    ax[3, 0].set(
        xlabel="time (s)", ylabel="ego velocity (m/s)", title="longitudinal velocity"
    )
    ax[3, 0].plot(time_ego, vel_egox, "r")

    ax[3, 1].set(
        xlabel="time (s)", ylabel="ego velocity (m/s)", title="lateral velocity"
    )
    ...
    ax[4, 1].set(
        xlabel="time (s)", ylabel="position - ego (m)", title="lateral position"
    )
    ...
    ax[5, 1].set(
        xlabel="time (s)", ylabel="position - 2nd (m)", title="lateral position"
    )
    ...
    ax[6, 1].set(
        xlabel="time (s)", ylabel="velocity - ego (m/s)", title="lateral velocity"
    )
    ax[6, 1].plot(time_ego, vel_egoy, "b")

    ax[7, 0].set(
        xlabel="time (s)", ylabel="velocity - 2nd (m)", title="longitudinal velocity"
    )
    ax[7, 0].plot(listtime, velx_veh2, "r")

    ax[7, 1].set(
        xlabel="time (s)", ylabel="velocity - 2nd (m)", title="lateral velocity"
    )
