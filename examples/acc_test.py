...
def main():
    ...
    # Create the vehicles.
    vehicle1 = Vehicle('ego_vehicle', model='etk800', licence='ego vehicle', color='Red')
    vehicle2 = Vehicle('other_vehicle', model='etk800', licence='vehicle2', color='Blue')

    # Create a scenario.
    scenario = Scenario('italy', 'ACCtest', description='ACC1')

    # Add the vehicles to the scenario.
    scenario.add_vehicle(vehicle1, pos=(-350.76326635601, 1169.0963008935, 168.6981158547), rot_quat=angle_to_quat((0, 0, 90)))
    scenario.add_vehicle(vehicle2, pos=(-365.56326635601, 1169.0963008935, 168.69811585470), rot_quat=angle_to_quat((0, 0, 90)))
    scenario.make(bng)
    ...
    ax[0].set(xlabel ='time (s)', ylabel='distance (m)', title='longitudinal relative distance')
    ax[0].plot(listtime, listrelDistX, 'r')



    
    ax[1].set(xlabel ='time (s)', ylabel='position - ego (m)', title='longitudinal - position')
    ax[1].plot(time_ego, posx_ego, 'r')


    ax[2].set(xlabel ='time (s)', ylabel='position - 2nd (m)', title='longitudinal - position')
    ax[2].plot(listtime, posx_veh2, 'r')


    
    ax[3].set(xlabel ='time (s)', ylabel='velocity (m/s)', title='longitudinal relative velocity')
    ax[3].plot(listtime, listrelVelX, 'r')

    ax[4].set(xlabel ='time (s)', ylabel='velocity - ego (m/s)', title='longitudinal velocity')
    ax[4].plot(time_ego, vel_egox, 'r')


    ax[5].set(xlabel ='time (s)', ylabel='velocity - 2nd (m/s)', title='longitudinal velocity')
